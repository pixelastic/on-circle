...
current.repo = new Gilmore(tmpDirectory('on-circle/gitCommit'));
...
describe('gitCommit', () => {
  ...
  describe('run', () => {
    ...
    describe('with real git repo', () => {
      beforeEach(async () => {
        await emptyDir(current.repo.root);
        await current.repo.init();
      });
      afterEach(async () => {
        await emptyDir(current.repo.root);
      });
      it('should save git user.name and git user.email as env variables', async () => {
        ...
        await current.repo.setConfig('user.name', 'OnCircle');
        await current.repo.setConfig('user.email', 'oncircle@test.com');
